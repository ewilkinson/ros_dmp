...
class RhythmicDMP(DMPs):
    """An implementation of rythmic DMPs"""
    ...
    def __init__(self, **kwargs): 
        ...
        super(RhythmicDMP, self).__init__(pattern='rhythmic', **kwargs)
    ...
    def gen_centers(self):
        ...
        c = np.linspace(0, 2*np.pi+0.2, self.bfs+1)
        c = c[0:-1]
        self.c = c
...
if __name__ == "__main__":
    ...
    # test normal run
    dmp = RhythmicDMP(ID='non_forcing',dmps=1, bfs=10, w=np.zeros((1,10)))
    ...
    for ii, bfs in enumerate(num_bfs):
        dmp = RhythmicDMP(ID='asdf',dmps=2, bfs=bfs)

        dmp.imitate_path(y_des=np.array([path1, path2]))
        # change the scale of the movement
        #dmp.goal[0] = 3; dmp.goal[1] = 2

        y_track,dy_track,ddy_track = dmp.rollout(tau=1.0, timesteps=len(path1))
