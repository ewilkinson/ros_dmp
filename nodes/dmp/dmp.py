...
class DMPs(object):
    ...
    def gen_sum_psi(self, psi, dmp_num):
        """ 
        Compute the normalized sumation of the psi's and their weights
        
        psi list/array: the activation of each basis function
        dmp_num int:the dmp number 
        """
        
        return np.dot(psi, self.w[dmp_num]) / np.sum(psi)
    ...
    def imitate_path(self, y_des):
        ...
        for d in range(self.dmps):
            ...
            for t in range(self.timesteps):  
                path[d, t] = path_gen(t * self.cs.dt)
        y_des = path

        # calculate velocity of y_des
        dy_des = np.diff(y_des) / self.cs.dt
        # add zero to the beginning of every row
        dy_des = np.hstack((np.zeros((self.dmps, 1)), dy_des))

        # calculate acceleration of y_des
        ddy_des = np.diff(dy_des) / self.cs.dt
        # add zero to the beginning of every row
    ...
    def rollout(self, timesteps=None, **kwargs):
        ...
        y_track = np.zeros((self.dmps, timesteps)) 
        dy_track = np.zeros((self.dmps, timesteps))
        ddy_track = np.zeros((self.dmps, timesteps))
    ...
    def step(self, tau=1.0, state_fb=None, dt=None):
       ...
       state_fb np.array: optional system feedback
        ...
        if state_fb is not None: 
            ...
            cs_args['error']  = np.sqrt(np.sum((state_fb - self.y)**2))
        
        #update tau from Canonical System error
        ...
        for d in range(self.dmps):
            ...
            f = self.gen_front_term(x, d) * self.gen_sum_psi(psi, d)

            # DMP acceleration
            self.ddy[d] = (self.ay[d] * 
                     (self.by[d] * (self.goal[d] - self.y[d]) - \
                     self.dy[d]/tau) + f) * tau**2
            self.dy[d] += self.ddy[d] * dt * self.cs.error_coupling
            self.y[d] += self.dy[d] * dt * self.cs.error_coupling
